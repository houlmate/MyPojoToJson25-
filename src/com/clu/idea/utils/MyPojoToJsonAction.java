...
public class MyPojoToJsonAction extends DumbAwareAction {
    ...
    public void update(@NotNull AnActionEvent e) {
        ...
        // Keep the action always available in Java files; actual type validation happens on execution.
        ...
        ProgressManager.getInstance().run(new Task.Backgroundable(project, "Converting " + className + " to JSON...") {
            ...
            public void run(@NotNull ProgressIndicator indicator) {
                ...
                ApplicationManager.getApplication().invokeLater(() -> {
                    CopyPasteManager.getInstance().setContents(new StringSelection(finalJson));
                    LOG.info("MyPojoToJson: conversion success for class " + className + ", json length=" + finalJson.length());
